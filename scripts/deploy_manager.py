...
def deploy(network, provider, account):
    ...
    if EXISTING_RECOVERY_GAUGE is None:
        recovery_gauge = account.deploy(project.RecoveryGauge, REWARD_TOKEN, RECOVERY_ADDRESS, max_priority_fee="1000 wei", max_fee="0.15 gwei", gas_limit="100000")
    else:
    ...
    deploy = account.deploy(project.RewardManager, managers, REWARD_TOKEN, gauges, RECOVERY_ADDRESS, max_priority_fee="1000 wei", max_fee="0.15 gwei", gas_limit="400000")
