...
def test_reward_manager_receivers(bob, test_gauge, reward_manager):
    receiving_gauges = reward_manager.receiving_gauges(0)
...
def test_reward_manager_send_reward_token(alice, bob, diana, reward_token, test_gauge, reward_manager):
    amount = 10 ** 16
    ...
    reward_manager.send_reward_token(test_gauge, amount, sender=bob)
...
def test_reward_manager_send_reward_token_epoch(alice, bob, diana, reward_token, test_gauge, reward_manager):
    amount = 10 ** 16
    reward_manager_balance = reward_token.balanceOf(reward_manager, sender=alice)
    assert reward_manager_balance == 0
    reward_token.transferFrom(bob, reward_manager, amount, sender=bob)
    reward_manager_balance = reward_token.balanceOf(reward_manager, sender=alice)
    assert reward_manager_balance == amount
    reward_manager.send_reward_token(test_gauge, amount, 8 * DAY, sender=bob)
    assert test_gauge.recover_token(sender=alice)
    balance_recoverd = reward_token.balanceOf(diana, sender=alice)
    assert amount == balance_recoverd
    assert amount == reward_manager_balance

def test_reward_manager_sent_revert(alice, test_gauge, reward_manager):
    with ape.reverts("only reward managers can call this function"):
        reward_manager.send_reward_token(test_gauge, 10 ** 18, sender=alice)

def test_reward_manager_epoch_revert_too_short(bob, test_gauge, reward_manager):
    with ape.reverts("epoch duration must be between 3 days and a year"):
        reward_manager.send_reward_token(test_gauge, 10 ** 18, DAY, sender=bob)

def test_reward_manager_epoch_revert_too_long(bob, test_gauge, reward_manager):
    with ape.reverts("epoch duration must be between 3 days and a year"):
        reward_manager.send_reward_token(test_gauge, 10 ** 18, 53 * WEEK, sender=bob)
