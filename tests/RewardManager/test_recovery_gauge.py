...
def test_recovery_gauge_recovery_address(diana, recovery_gauge):
    assert recovery_gauge.recovery_address() == diana
...
def test_recovery_gauge_recover_token(alice, bob, diana, recovery_gauge, reward_token):
    balance = reward_token.balanceOf(bob)
    assert reward_token.approve(recovery_gauge, balance, sender=bob)
    recovery_gauge.deposit_reward_token(reward_token, balance, sender=bob)
